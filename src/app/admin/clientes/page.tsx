...
export default function AdminClientes() {
  ...
  useEffect(()=>{ if (!isStaff) { router.replace("/login"); return; } cargar(); },[isStaff]);

  const cargar = async (q="") => {
    setLoading(true);
    const r = await api<any[]>(`/admin/clientes${q?`?buscar=${q}`:""}`).catch(()=>[]);
    setClientes(r); setLoading(false);
  };
      ...
      <main className="admin-main overflow-y-auto">
        <div className="p-7">
          ...
          <div className="flex gap-3 mb-5">
            <input value={buscar} onChange={e=>setBuscar(e.target.value)} onKeyDown={e=>e.key==="Enter"&&cargar(buscar)}
              placeholder="Nombre, documento, email..." className="input max-w-xs text-[13px]"/>
                ...
                <tbody>
                  {clientes.map(c=>(
                    <tr key={c.id_cliente} className="border-b border-[#0f0f0f] hover:bg-[#0f0f0f]">
                      ...
                      <td className="px-4 py-3"><span className={`badge ${c.segmento==="vip"?"badge-gold":c.segmento==="frecuente"?"badge-green":"badge-gray"} capitalize`}>{c.segmento}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
              {!clientes.length && <p className="text-center py-10 text-[#333] text-[12px]">Sin clientes</p>}
            </div>
          )}
        </div>
      </main>
